...
export const addToList = async (req, res) => {
    try {
        ...
        const validStatuses = ["watchlist", "watched", "favorites"];
        ...
        const exists = await List.findOne({ user: userId, tmdbId: id, type, status });
        if (exists) return res.status(400).json({ error: "Movie already in list" });

        const newListItem = new List({ user: userId, tmdbId: id, type, status });
...
export const removeFromList = async (req, res) => {
    try {
        ...
        const removed = await List.findOneAndDelete({ user: userId, tmdbId: id, type, status });
